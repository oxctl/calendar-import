...
public class UserRepositoryTest {
    ...
    public void testFindByTenantNameAndUsername() throws Exception {
        ...
        entityManager.flush();
        User user = this.repository.findByUsernameAndTenant_Name("username", "tenant").orElseThrow(AssertionFailedError::new);
    ...
    public void testTenentIsolation() throws Exception {
        ...
        {
            User user = repository.findByUsernameAndTenant_Name("username", "tenant").orElseThrow(AssertionFailedError::new);
            assertEquals("tenant", user.getTenant().getName());
            assertEquals("username", user.getUsername());
        }
        {
            User user = repository.findByUsernameAndTenant_Name("username", "other").orElseThrow(AssertionFailedError::new);
