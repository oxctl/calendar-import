...
public class CSVCalendarImportJobTest {
    ...
    public void testSingleEvent() throws IOException {
        List<CalendarEvent> calendarEvents = csvImportJob.parseCSV(getClass().getResource("/one-event.csv"));
    ...
    public void testMultipleEvents() throws IOException {
        List<CalendarEvent> calendarEvents = csvImportJob.parseCSV(getClass().getResource("/two-events.csv"));
        assertFalse(csvImportJob.getErrors().hasNext());
        assertNotNull(calendarEvents);
    ...
    public void testIgnoredHeaders() throws IOException {
        List<CalendarEvent> calendarEvents = csvImportJob.parseCSV(getClass().getResource("/extra-headers.csv"));
        assertFalse(csvImportJob.getErrors().hasNext());
        assertNotNull(calendarEvents);
        assertEquals(1, calendarEvents.size());
        CalendarEvent event = calendarEvents.get(0);
    ...
    public void testIgnoredRowValues() throws IOException {
        List<CalendarEvent> calendarEvents = csvImportJob.parseCSV(getClass().getResource("/extra-values.csv"));
        assertFalse(csvImportJob.getErrors().hasNext());
        assertNotNull(calendarEvents);
        assertEquals(1, calendarEvents.size());
        CalendarEvent event = calendarEvents.get(0);
