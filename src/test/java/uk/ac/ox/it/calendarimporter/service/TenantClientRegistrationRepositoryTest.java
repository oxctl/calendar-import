...
public class TenantClientRegistrationRepositoryTest {
  ...
  @Autowired private TenantClientRegistrationRepository clientRegistrations;
  ...
  public void testFind() {
    ...
    assertEquals(
        "https://example.com/login/oauth2/auth",
    ...
    assertEquals(
        "{baseUrl}/login/oauth2/code/{registrationId}", registration.getRedirectUriTemplate());
    assertEquals(
        "https://example.com/login/oauth2/token", registration.getProviderDetails().getTokenUri());
    assertEquals(
        "https://example.com/api/v1/users/self",
