...
public class OAuth2ErrorController {
    ...
    public void handleError(@RequestParam(value = "error", required = false) String errorCode,
                @RequestParam(value = "error_description", required = false) String errorDescription,
                HttpServletRequest request, HttpServletResponse response) throws OAuth2AccessDeniedException, AuthenticationException {
        ...
        if ("access_denied".equals(errorCode)) {
            ...
            throw new OAuth2AccessDeniedException("You must allow access to this tool to use it.", redirectUrl);
                ...
                throw new AuthenticationServiceException("Developer keys are misconfigured, check configuration.");
        } else {
            // There is no validation that these parameters are true so we shouldn't output to the end user.
