...
public class IcalImportJob extends CanvasCalendarJob {
    ...
    public void run() throws IOException {
        ...
        CalendarReader reader = canvasApiFactory.getReader(CalendarReader.class, nonRefreshableOauthToken, PAGINATION_PAGE_SIZE);
        CalendarWriter writer = canvasApiFactory.getWriter(CalendarWriter.class, nonRefreshableOauthToken);
        ...
        List<CalendarEvent> calendarEvents = reader.listCurrentUserCalendarEvents(listCalendarEventsOptions);
        ...
        try (InputStream in = new TerminatingInputStream( url.openStream(), inputLimit)) {
            ...
            for (VEvent event : events) {
                ...
                if (uid != null) {
                    ...
                    Set<Integer> canvasIDs = (canvasEvenets != null)?
                    ...
                    if (canvasIDs != null) {
                        for (Integer id : canvasIDs) {
                            ...
                            if (oldHash != calendarEvent.hashCode()) {
                                ...
                                returned.ifPresent(calendarEvent1 -> idLookup.set(uid.getValue(), Collections.singleton(calendarEvent1.getId())));
                            }
                        }
                    } else {
        ...
        // Summary of run.
        logger.info("Events in source: "+ calendarEvents.size()+ " seen: "+ seen+ " new: "+ newEvents+ " updated: "+ updatedEvents);
    ...
    public void update(VEvent vevent, CalendarEvent event) {
        ...
        if (description != null) {
            ...
            escapedDescription = HiddenData.insertHidden(escapedDescription, HiddenData.toHidden(vevent.getUid().getValue()));
