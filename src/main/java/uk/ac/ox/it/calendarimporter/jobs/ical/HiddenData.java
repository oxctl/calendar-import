...
public class HiddenData {
    ...
    private static final String prefix = "<div class=\"calendar-data-1\" style=\"display: none;\" data-calendar=\"";
    ...
    public static String fromHidden(String comment) {
        if (comment != null && comment.startsWith(prefix) && comment.endsWith(suffix)) {
            byte[] decoded = Base64.getDecoder().decode(comment.substring(prefix.length(), comment.length() - suffix.length()));
