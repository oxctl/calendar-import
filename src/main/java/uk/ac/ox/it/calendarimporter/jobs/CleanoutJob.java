...
public class CleanoutJob implements Job {
  ...
  public void execute(JobExecutionContext jobContext) throws JobExecutionException {
    ...
    UserTokens userTokens = userTokensRepository.findById(tenantUser)
            .orElseThrow(() -> new JobExecutionException("Couldn't find tokens for: "+tenantUser));
    OauthToken oauthToken = oauthTokenFactory.getToken(
            tenant,
            tenantUser,
    ...
    CalendarReader calendarReader =
        canvasApiFactory.getReader(CalendarReader.class, oauthToken);
    ...
    try {
      ...
      for (CalendarEvent event : calendarEvent) {
        if (HiddenData.extractHidden(event.getDescription()) != null) {
          ...
          log.debug(
                  "Ignoring event ID {} from calendar {} of {}",
                  event.getId(),
                  context,
                  tenant);
        }
      }
      log.info("Removed {} or {} events from calendar {} of {}", removed, calendarEvent.size(), context, tenant);
    } catch (IOException e) {
