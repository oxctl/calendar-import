...
/**
 * We want to store refreshed tokens.
 * This has the problem that if we allow tokens to be used from live on beta/test.
 * Then when the tokens get updated live will break.
 */
@Service
public class OauthTokenFactory {
    ...
    public OauthTokenRefresher getRefresher(String principal, String clientId, String clientSecret, String url) {
    ...
    public OauthToken getToken(Authentication authentication, OAuth2AuthorizedClient client) {
        ...
        if (client.getRefreshToken()!= null && client.getRefreshToken().getTokenValue() != null) {
            ...
            StoredOauthTokenRefresher refresher = new StoredOauthTokenRefresher(
                    authentication.getPrincipal().toString(),
                    registration.getClientId(),
                    registration.getClientSecret(),
                    url);
            return new RefreshableOauthToken(refresher, client.getRefreshToken().getTokenValue(), client.getAccessToken().getTokenValue());
        }
        return new NonRefreshableOauthToken(client.getAccessToken().getTokenValue());
    }

    public OauthToken getToken(Tenant tenant, String principal, String access_token, String refresh_token) {
    ...
    /**
     * Just removes the local part from a URL. This is just needed so we don't need more configuration.
     * In the long run we should update the canvas-api library to take the full token URL.
    ...
    public class StoredOauthTokenRefresher extends OauthTokenRefresher {
        ...
        public StoredOauthTokenRefresher(String principal, String clientId, String clientSecret, String canvasUrl) {
