...
/**
 * This holds the tokens for a user. Each token can be null, but if not null then they have to be valid.
 * TODO Should this be called PrincipalTokens
 */
@Entity
@Data
@NoArgsConstructor
...
public class UserTokens {
    ...
    @EmbeddedId
    private TenantAndPrincipal tenantAndPrincipal;
    ...
    public UserTokens(OAuth2AuthorizedClient oauth2AuthorizedClient) {
        this.tenantAndPrincipal = new TenantAndPrincipal(oauth2AuthorizedClient.getClientRegistration().getRegistrationId(), oauth2AuthorizedClient.getPrincipalName());
    ...
    public OAuth2AuthorizedClient toOAuth2AuthorizedClient(ClientRegistration client, String principal) {
    ...
    /**
     * We pack the scopes into a string as they are space seperated according to the spec so we can re-use that
     * fact.
     *
     * @see <a href="https://tools.ietf.org/html/rfc6749#section-3.3">RFC-6749 Section 3.3</a>
     */
    @Data
    @Embeddable
    @NoArgsConstructor
    ...
    public static class AccessToken {
        ...
        public OAuth2AccessToken toOAuth2AccessToken() {
            Set<String> scopes = (this.scopes != null && !this.scopes.isEmpty())
                    ? new HashSet<>(Arrays.asList(this.scopes.split(" ")))
                    : Collections.emptySet();
            OAuth2AccessToken oauth2token = new OAuth2AccessToken(BEARER, tokenValue, issuedAt, expiresAt, scopes);
            return oauth2token;
        }
    }
