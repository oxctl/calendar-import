...
public class CustomPathCookieSerializer implements CookieSerializer {
  ...
  private static final Log logger = LogFactory.getLog(CustomPathCookieSerializer.class);
  ...
  public static final String ADDITIONAL_PATH = CustomPathCookieSerializer.class.getName();
  ...
  /**
   * Set if the Base64 encoding of cookie value should be used. This is valuable in order to support
   * <a href="https://tools.ietf.org/html/rfc6265">RFC 6265</a> which recommends using Base 64
   * encoding to the cookie value.
   *
  ...
  private String getCookiePath(HttpServletRequest request) {
    // This appends on any custom parts to the cookie path
    HttpSession session = request.getSession(false);
    String additionalPath = null;
    if (session != null) {
      Object attribute = session.getAttribute(ADDITIONAL_PATH);
      if (attribute != null) {
        additionalPath = attribute.toString();
      }
    }
