...
/**
 * This persists the OAuth2 tokens in the DB, this means we don't have to get the user to authenticate
 * each time they use the tool.
 */
@Component
public class UserOAuth2AuthorizedClientRepository implements OAuth2AuthorizedClientRepository {
    ...
    public <T extends OAuth2AuthorizedClient> T  loadAuthorizedClient(String clientRegistrationId, Authentication authentication, HttpServletRequest request) {
        ...
        if (clientRegistration != null) {
            ...
            oAuth2AuthorizedClient = userTokensRepository.findById(new TenantAndPrincipal(clientRegistrationId, principal)).map(
                    userTokens -> userTokens.toOAuth2AuthorizedClient(clientRegistration, principal)
            ).orElse(null);
        }
    ...
    @Override
    public void removeAuthorizedClient(String clientRegistrationId, Authentication authentication, HttpServletRequest request, HttpServletResponse response) {
        userTokensRepository.deleteById(new TenantAndPrincipal(clientRegistrationId, authentication.getPrincipal().toString()));
    }
}
