...
/**
 * This allows loading of sections that are in a course.
 * This is developed as an API so that the page can load quickly and doesn't hang about waiting for an
 * API call from Canvas to come back.
 */
@RestController
...
public class SectionsController {
  ...
  // TODO This should be cached for 5 minutes so that when the page reloads the sections doesn't load again.
  ...
  public List<CourseSection> getSections(
    ...
    NonRefreshableOauthToken token = new NonRefreshableOauthToken(
    ...
    List<CourseSection> courseSections = sections.stream()
        .map(s -> new CourseSection("course_section_" + s.getId(), s.getName())).collect(
            Collectors.toList());
