...
import static uk.ac.ox.it.calendarimporter.xml.CartridgeLtiLink.Extensions;
import static uk.ac.ox.it.calendarimporter.xml.CartridgeLtiLink.Options;
import static uk.ac.ox.it.calendarimporter.xml.CartridgeLtiLink.Property;
...
/**
 * Just provides a simple dynamic XML file for LTI tool registration. This allows XML configuration
 * to be set for any host without editing as it pulls the host from the current request.
 */
@RestController
public class ConfigController {
  ...
  public CartridgeLtiLink getConfig(HttpServletRequest request) {
    ...
    String launchUrl =
        ServletUriComponentsBuilder.fromContextPath(request).path(ltiLaunchPath).toUriString();
